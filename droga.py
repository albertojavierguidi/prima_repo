...
# we have to find a way to define a step sequence to  decide wether
# to make an instrument play or not.

lead_sequence = [0, None, 2, None, 4, None, 0, None] * 2 # None is used to insert some pauses.
hihat_sequence = [1, 1, 1, 1] * 4
kick_sequence = [1, 0] * 4

# we have to pass these sequencies to a sequencer.

def silence(duration):
    return [0.0] * int(sample_rate * duration)

def lead_step(step, duration): # takes as input the value in the step and its duration (which well be the same during the whole track)
    if step is None:
        return silence(duration)
    else:
        return envelope_wave(create_sawtooth_wave(440 * interval_to_freq(step), duration))
...
sound_wave = mix_many_waves([lead_wave, hihat_wave, kick_wave])

max_val = max(abs(x) for x in sound_wave)
sound_wave = [ sample/max_val for sample in sound_wave]
